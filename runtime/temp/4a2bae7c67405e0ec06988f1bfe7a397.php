<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:68:"E:\wamp\www\Blogs\public/../application/index\view\index\fenlei.html";i:1519430674;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <script>
        function ajax_page(page) {
            $.ajax({
                url:"<?php echo url('Index/ajax_rel'); ?>",
                type:"post",
                data:{page:page,biao:"<?php echo $biao; ?>"},
                success:function (data) {
                    var add_html="<p class='main_left_zx'>"+data.list.data[0]["lei"]+"</p>";
                    var list = data.list.data;
                    var html_page = data.page;
                    for (var i=0;i<list.length;i++){
                        add_html=add_html+"<article class='main_left_article'><h3 class='main_left_h'><a href='_BLOGS_/index/show?biao="+data.biao+"&&id="+list[i]["id"]+"' target='_blank'>"+list[i]["arti_title"]+"</a></h3><p><i>"+list[i]["ad_time"]+"</i><i>阅读："+list[i]["clicksum"]+"</i><i>"+list[i]["lei"]+"</i></p><div class='main_left_p'>"+list[i]["arti_contents"]+"</div></article>";
                    }
                    $(".main_left_main").html(add_html);
                    $(".main_left_fenye").html(html_page);
                }
            });
        }
    </script>
...
<div class="con">
    ...
    <div class="main">
        ...
        <div class="main_left">
            ...
            <p class="main_left_zx"><?php echo $Info['0']['lei']; ?>:</p>
            <?php if(is_array($Info) || $Info instanceof \think\Collection || $Info instanceof \think\Paginator): $i = 0; $__LIST__ = $Info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
            <article class="main_left_article">
                <h3 class="main_left_h"><a href="./show?biao=<?php echo $biao; ?>&&id=<?php echo $vo['id']; ?>" target="_blank"><?php echo $vo['arti_title']; ?></a></h3>
                <p>
            ...
            <div class="main_left_fenye"><?php echo $Info->render(); ?></div>
        </div>
        <!--右-->
