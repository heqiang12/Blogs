...
class Ajaxpage extends Paginator
{
    //首页
    protected function home() {
        if ($this->currentPage() > 1) {
//            return "<a href='" . $this->url(1) . "' title='首页'>首页</a>";
            return '<a href="javascript:ajax_page('. (1) .')">首页</a>';//这里我修改了路径
        } else {
            return "<p>首页</p>";
        }
    }
...
//            return "<a href='" . $this->url($this->currentPage - 1) . "' title='上一页'>上一页</a>";
            return '<a href="javascript:ajax_page('. ($this->currentPage() - 1) .')" title="上一页">上一页</a>';//这里我修改了路径
        } else {
            return "<p>上一页</p>";
        }
    }
...
//            return "<a href='" . $this->url($this->currentPage + 1) . "' title='下一页'>下一页</a>";
            return '<a href="javascript:ajax_page('. ($this->currentPage() + 1) .')" title="下一页">下一页</a>';//这里我修改了路径
        } else {
            return"<p>下一页</p>";
        }
    }
...
//            return "<a href='" . $this->url($this->lastPage) . "' title='尾页'>尾页</a>";
            return '<a href="javascript:ajax_page('. ($this->lastPage) .')" title="尾页">尾页</a>';//这里我修改了路径
        } else {
            return "<p>尾页</p>";
        }
    }
    ...
    protected function getLinks()
    {
        ...
        if ($this->lastPage < $window + 6) {
            ...
            //原本为getUrlRange($this->lastPage -1 , $this->lastPage),这里我修改了样式
        } elseif ($this->currentPage > ($this->lastPage - $window)) {
            $block['first'] = $this->getUrlRange(1, 1);
            //原本为getUrlRange(1,2),这里我修改了样式
...
//        return '<a href="' . htmlentities($url) . '" title="第"'. $page .'"页" >' . $page . '</a>';
        return '<a href="javascript:ajax_page('.$page.');" title="第"'.$page.'"页">' . $page . '</a>';
    }
    /**
     * 生成一个禁用的按钮
     *
